...
def bytes_to_gb(b: int) -> float:
    return b / (1024 ** 3) #converting to gigabytes

def gb_to_energy_kwh(gb: float) -> float:
    return gb * KWH_PER_GB #calculating energy consumed

def energy_to_co2_kg(energy_kwh: float) -> float:
    return energy_kwh * KG_CO2_PER_KWH #calculates the carbon emission
